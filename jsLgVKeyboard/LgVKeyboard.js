...
//								lgKb.changeLangJs(lgKb.nextLang);		//	this is called HIDLanguageChange event
	        ...
	        function (event)
	        {
               if(lgKb.targetElement)
               {
                       //      add your codes
                              ...
                              && (!event.target.readOnly) && (!event.target.disabled) && (TVA.nativeKeyboard))
               {
                       lgKb.focusIn(event);
               }
	        },
		WindowFocusOut :
			function (event)
			{
				...
				{
					// Commented out because it was causing multiple calls to lgKb.focusOut()
					//TVA.log("## WindowFocusOut > focusOut()");
					//lgKb.focusOut();
				}	
			},
		onClearCaretInfo :
...
//						return;		//	Virtual Keyboard works well on LG Smart TV
		...
		focusOut :
			...
			{	
				...
				if(lgKb.targetElement)
				{
					if(lgKb.bShowPopupLangSel || lgKb.bMiniPopupshow){
						if(lgKb.bShowPopupLangSel)
						{
							TVA.log("## CLOSING LANGUAGE POPUP");
							lgKb.showPopupLangSel(false);
							TVA_Input.langScreen = false;
						}
						...
						lgKb.onKeyboardHide();
						/////////////////////////////////////////////////// 
						// Added to simulate an onEnter event
						//alert('ping');
						
						//TVA_Input.closeInput();
						///////////////////////////////////////////////////
					}
				}
			},
		...
		showPopupLangSel :
			function (bShow)
			{
				...
				TVA_Input.langScreen = bShow;
				if(bShow)
				{
					TVA.log("show lanuguage window");
					lgKb.popupInit(true);
		...
		cleanKeyboard :
			function (event)
			{
				...
				{
					TVA.log("## cleanKeyboard > focusOut()");
					lgKb.focusOut();
				}
			},
		setSelLang :
					...
					switch(keyId)
					{
						...
						case 'wkk_key_spacebar' :
							...
							if (typeof onCharacter =="function") {
								onCharacter(TVA_Input.inputBox.value);
							}
							break;
						case 'wkk_key_hide' :
							TVA.log("## wkk_key_hide > focusOut()");
							TVA.wasActiveInput = true;
							TVA_Input.closeInput();
							break;
						default : 
	...
	if(new String(navigator.userAgent).search(/LG NetCast.*-2011/i) != -1) {
		// browser doesn't support focusin and focusout events...		
